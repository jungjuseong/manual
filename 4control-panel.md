...
컨트롤 패널은 최상단의 탭(홈, 삽입, 보기)과 선택한 객체(도형, 이미지, 동영상 등)에 따라 다른 형태의 리본 메뉴를 보여줍니다. 


또한 선택한 객체(도형, 이미지, 동영상 등)에 따라 속성값을 수정할 수 있는 속성 화면?? 메뉴 화면??이 나타납니다.  
...
<img src='./figure/4-6_깸.jpg' width="600" >
...
이미비 박스 선택후 이미지 박스 서식 메뉴에서 왼쪽에 있는 파일 아이콘을 선택하면 이미지 열기가 가능합니다. 
...
이미지 박스 서식 메뉴에서 '크롭'을 선택한 후 이미지 포인터를 드래그하여 이미지 크기를 조정하면 이미지 사이즈가 조절됩니다.
